// Sockets are made non blocking by the fcntl() call in set_non_blocking()
// cfd and sfd are made non blocking
// Theyre made non blokcing to avoid the server freezing up during certian function calls
...
it is passed a type struct acceptor_args with addresses and mutexes.
2. How are received messages stored?
In a linked list
3. What does `main()` do with the received messages?
it iterates through the list printing messages and eventually freeing them
4. How are threads used in this sample code?
threads are used for accept
*/ 
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
