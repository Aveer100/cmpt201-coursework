...
int main() {
  ...
  FILE* myf = fopen("public_key.pem", "r");
  pubkey = PEM_read_PUBKEY(myf, NULL, NULL, NULL);
  fclose(myf);
...
#define MAX_FILE_SIZE 512
  ...
  //unsigned char msg[MAX_FILE_SIZE];
  //unsigned char sig[MAX_FILE_SIZE];
  FILE* myf = fopen(message_path, "r");
  size_t len1 = fread(message, 1, MAX_FILE_SIZE, myf);
  fclose(myf);
  myf = fopen(sign_path, "r");
  size_t len2 = fread(signature, 1, MAX_FILE_SIZE, myf);
  EVP_MD_CTX* digest = EVP_MD_CTX_new();
  int temp = 0;
  if(EVP_DigestVerifyInit(digest, NULL, EVP_sha256(), NULL, pubkey) == 1)
  {
    if(EVP_DigestVerifyUpdate(digest, message, len1) == 1)
    {
      temp = EVP_DigestVerifyFinal(digest, signature, len2);
    }
